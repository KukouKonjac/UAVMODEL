...
class Comm {
  public:
    ...
    bool SetInput(const facilityPOS& postion, const Env& environment, const bool& jam, const carrier& vehicle);
  ...
  private:
    ...
    Env env;
    double NJ;        // 干扰噪声
    double Pnj_r;     // 接收机接收到的干扰噪声
    double Pnj_t;     // 干扰机发射功率
    double reld_N;    // 干扰机与接收机的距离
    double Nt;        // 环境噪声
    double N0;        // 总噪声
