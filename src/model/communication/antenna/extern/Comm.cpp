...
bool Comm::Comm_test(double Posa_x, double Posa_y, double Posa_z, double Posb_x, double Posb_y, double Posb_z,
                     double FT_comm) {
    ...
    // bool jam = true;
    //     double jam = 1e-16;
    double k = 1.380649e-23; // Boltzmann 常数
    carrier plat1;
    plat1.psi = 0.0;
    plat1.thelta = 0.0;
    plat1.phi = 0.0;
...
bool Comm::SetInput(const facilityPOS& postion, const Env& environment, const bool& j, const carrier& vehicle) {
    ...
    // NJ = 10 * log10(j);
    // // Pnj_r = Pnj_t - P_Loss - Atmos() * dnj / 1e03 + Gj;
    if (j) {
        double P_Loss = 32.45 + 20 * log10(reld_N) + 20 * log10(tran_para.Comm_Set_Freq);
        // Pnj_t = 10 * log10(Pnj_t * 1e3); // 干扰发射功率W转换dBm
        Pnj_t = 100; // 干扰发射功率W转换dBm
        Pnj_r = Pnj_t - P_Loss - Atmos() * reld_N / 1e03;
    } else {
        Pnj_r = 0;
    }
...
facilityPOS Comm::Get_tranpos() { return this->comm_pos; }

double Comm::Get_tranp() { return this->tran_para.Comm_Set_Power; }

double Comm::Get_trangt() { return this->Gt; }

double Comm::Get_trangtf() { return this->tran_para.Comm_Set_Freq; }

double Comm::Get_trangB() { return this->tran_para.Comm_Set_Freq_Bw; }
...
void Comm::tran_Gain() { Gt = Gain(tran_para.Comm_Set_Freq, tran_para.Comm_Set_i, tran_para.Comm_Set_A); }

void Comm::rec_Gain() { Gr = Gain(rec_para.Comm_Set_Freq, rec_para.Comm_Set_i, rec_para.Comm_Set_A); }
...
double Comm::Atmos() {
    Atmosphere A;
    double T = 288.16 - 0.0065 * (comm_pos.Comm_Set_altitude) / (1 + (comm_pos.Comm_Set_altitude) / 637.81 * 1e04);
    ...
    double eN;
    // 氧气的谐振频率
    unordered_map<int, pair<double, double>> fN = {
        {1, {56.2648, 118.7507}}, {3, {58.4466, 62.4863}},  {5, {59.5910, 60.3601}},  {7, {60.4348, 59.1642}},
        {9, {61.1506, 58.3239}},  {11, {61.8002, 57.6125}}, {13, {62.4112, 56.9682}}, {15, {62.9980, 56.3634}},
        {17, {63.5685, 55.7839}}, {19, {64.1272, 55.2214}}, {21, {64.6779, 54.6728}}, {23, {65.2204, 54.1294}}};
    // 电磁波频率
    double f = rec_para.Comm_Set_Freq;
    double gh;
    if (comm_pos.Comm_Set_altitude <= 8)
        gh = 0.640;
    else
        gh = 0.3026 + 0.04218 * comm_pos.Comm_Set_altitude;
    double deltaf = 0.1895 * gh * P / T;
    F0 = deltaf / (f * f + deltaf * deltaf);
    for (N = 1; N <= 23; N = N + 2) {
        double n = static_cast<double>(N);
        eN = 2.06844 * n * (n + 1.0);
        uN_p = n * (2 * n + 3) / (n + 1), uN_n = (n + 1) * (2 * n - 1) / n,
        uN0 = 2 * (n * n + n + 1) * (2 * n + 1) / (n * (n + 1));
...
void Comm::GAIN() {
    ...
    double xb = 1.0, yb = tan(rec_para.Comm_Set_azi / 180 * 3.1415926),
           zb = tan(rec_para.Comm_Set_alt / 180 * 3.1415926) *
                pow((1 + tan(rec_para.Comm_Set_azi / 180 * 3.1415926) * tan(rec_para.Comm_Set_azi / 180 * 3.1415926)),
                    0.5);
    Matrix<double> PB(3, 1);
    PB[0][0] = xb, PB[1][0] = yb, PB[2][0] = zb;
    Matrix<double> S_thelta_psi_phi(3, 3);
    S_thelta_psi_phi[0][0] = cos(platform.thelta) * cos(platform.psi),
    S_thelta_psi_phi[0][1] = cos(platform.thelta) * sin(platform.psi), S_thelta_psi_phi[0][2] = -sin(platform.thelta),
    S_thelta_psi_phi[1][0] =
        sin(platform.thelta) * cos(platform.psi) * sin(platform.phi) - sin(platform.psi) * cos(platform.phi),
    S_thelta_psi_phi[1][1] =
        sin(platform.thelta) * sin(platform.psi) * sin(platform.phi) + cos(platform.psi) * cos(platform.phi),
    S_thelta_psi_phi[1][2] = cos(platform.thelta) * sin(platform.phi),
    S_thelta_psi_phi[2][0] =
        sin(platform.thelta) * cos(platform.psi) * sin(platform.phi) + sin(platform.psi) * cos(platform.phi),
    S_thelta_psi_phi[2][1] =
        sin(platform.thelta) * sin(platform.psi) * sin(platform.phi) - cos(platform.psi) * cos(platform.phi),
    S_thelta_psi_phi[2][2] = cos(platform.thelta) * cos(platform.phi);
...
std::unordered_map<double, bool> Comm::BER() // 计算误码率并将计算结果保存在map里
{
    ...
    // rel.d = 9500;
    double Path_Loss = 0;
    double dc = 4 * 3.1415926 * (3 * 3) / (3 * 1e8 / (1 * 1e8)); // 路径损耗模型选择判断距离 单位：m
    if (rel.d >= dc) {
        Path_Loss =
            40 * log10(rel.d) - (20 * log10(3) + 20 * log10(3)); // 双线反射模型的路径损耗 单位：dbm 3、3是天线高度
    } else {
        Path_Loss = 32.45 + 20 * log10(rel.d) + 20 * log10(tran_para.Comm_Set_Freq); // 自由空间损耗  单位：dBm、m、GHz
    }

    double B = tran_para.Comm_Set_Freq_Bw;
    double Rb = tran_para.Comm_Set_Rb;
    P_n = 10 * log10((1.38 * 1e-23) * rec_para.Comm_Set_T * B * 1e3) + 5 + Pnj_r;
    // 单位：dbm。热干扰噪声+背景（人为）噪声5db，未考虑电磁干扰NJ

    double P_t = 10 * log10(Comm_tran->Get_tranp() * 1e3); // dBm 发射信号功率
    P_r = P_t - Path_Loss - Atmos() * rel.d / 1e03 + G;    // 接收信号功率 dbm G为天线增益
    double SNR = P_r - P_n;                                // 单位：dB
    double ber = 0.5 * erfc(sqrt(SNR));                    // 单位：dB SNR=20db 《==》 ber=10e-10。
    if (ber > 1e-10)
        connect = false;
    else
        connect = true;
...
std::unordered_map<double, bool> Comm::GetBER() { return result; }
