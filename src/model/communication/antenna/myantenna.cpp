...
double MyAntenna::getcommPrPn(const Vector3& self, const Vector3& target, CommunicationData commdata) {
    using namespace externModel::comm;
    static Comm comm1, comm2;
    static Env environment{1, 1, 1};
    static carrier plat{0., 0., 0.};
    // static facilityPOS pos1;
    static DoJobOnConstruct init{[&]() {
        comm1.Init(commdata.transpower);
        comm2.Init(commdata.transpower);
        comm1.Comm_Init(&comm2);
        comm2.Comm_Init(&comm1);
    }};
    facilityPOS pos1, pos2;
    ...
    // cout << '\t' << comm1.Getber() << '\n' << endl;
    // cout << '\t' << comm2.Getber() << '\n' << endl;
    double Pr = comm2.GetPr();
    double Pn = comm2.GetPn();
    return Pr - Pn;
}
} // namespace uavmodel