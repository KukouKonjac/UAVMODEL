...
int main() {
    ...
    get<1>((*(model.components.getSpecificSingleton<uavmodel::ScannedMemory>()))[1]) =
        ...
        EntityInfo{.position = {3000, 0, 0},
                   .velocity = {0, 0, 0},
                   .baseInfo = {BaseInfo::ENTITY_TYPE::UAV, 1, 1, DAMAGE_LEVEL::N, 1, 1, /*0.6*/0.9, 3000.0}};
    auto& buffer = model.components.getSpecificSingleton<uavmodel::CommandBuffer>().value();
    buffer.emplace(static_cast<uavmodel::command::COMMAND_TYPE>(2), any(tuple<double, double>(0, 1)));
    double detectrate = 0;
    double jd = 0;
    for (int i = 0; i < 1000; ++i) {
        ...
        get<1>((*(model.components.getSpecificSingleton<uavmodel::ScannedMemory>()))[1]) =
            ...
            EntityInfo{.position = {3000, 0, 0},
                       .velocity = {0, 0, 0},
                       .baseInfo = {BaseInfo::ENTITY_TYPE::UAV, 1, 1, DAMAGE_LEVEL::N, 1, 1, 0.9, 3000.0}};
    }
    cout /*<< "探测距离："
                        << (get<1>(tmp).position -
